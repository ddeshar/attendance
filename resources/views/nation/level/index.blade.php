                ...
                <div class="x_panel">
                    <div class="x_title">
                        ...
                        <div class="clearfix">   <div class="pull-right">
                 	        	@can('role-create')
                 	       <a class="btn btn-primary" href="{{ route('roles.create') }}"> สร้างบทบาทใหม่</a>
                 	            @endcan
                 	        </div></div>
                    </div>
                    <div class="x_content">
...
<tbody>
  <?php $i = 0; ?>
	...
	<tr>
		<td>{{ ++$i }}</td>
		<td>{{ $levels->th_level }}</td>
		<td>{{ $levels->en_level }}</td>
		<td>
			{{-- <a class="btn btn-info" href="{{ route('level.show',$levels->id) }}">Show</a>
			@can('role-edit')
			<a class="btn btn-primary" href="{{ route('level.edit',$levels->id) }}">Edit</a>
			@endcan

			@can('role-delete')
			{!! Form::open(['method' => 'DELETE','route' => ['level.destroy', $levels->id],'style'=>'display:inline']) !!}
